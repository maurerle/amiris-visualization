...
from dashboard.data import Model, Column
from dashboard.data.files import CsvFile, FILES
...
class DataReader:
    ...
    def read_all(self, year: int) -> None:
        ...
        """
        self._populate(group="prices", amiris="DayAheadMarket", assume="MarketMeta", history="DaPrices", year=year,
                       column=Column.PRICE)
        self._populate(group="nuclear", amiris="Nuclear", assume="Nuclear", history="Dispatch", year=year,
                       column=Column.NUCLEAR)
        self._populate(group="lignite", amiris="Lignite", assume="Lignite", history="Dispatch", year=year,
                       column=Column.LIGNITE)
        self._populate(group="coal", amiris="Coal", assume="Coal", history="Dispatch", year=year, column=Column.COAL)
        self._populate(group="gas", amiris="Gas", assume="Gas", history="Dispatch", year=year, column=Column.GAS)
        self._populate(group="oil", amiris="Oil", assume="Oil", history="Dispatch", year=year, column=Column.OIL)
        self._populate(group="hydro", amiris="Hydro", assume="Hydro", history="Dispatch", year=year,
                       column=Column.HYDRO)
        self._populate(group="pv", amiris="PV", assume="PV", history="Dispatch", year=year, column=Column.PV)
        self._populate(group="onshore", amiris="Onshore", assume="Onshore", history="Dispatch", year=year, column=Column.ONSHORE)
        self._populate(group="offshore", amiris="Offshore", assume="Offshore", history="Dispatch", year=year,
                       column=Column.OFFSHORE)

    def _populate(self, group: str, amiris: str, assume: str, history: str, year: int,
                  column: Column) -> None:
        """
        ...
        """
        self._preparer.init_data_group(group=group, key_metadata={
            "TimeStamp": column_metadata(label="Simulation Time", unit="h"),
        })
        self._get_file(Model.AMIRIS, amiris, year).add_column(self._preparer, group, year, column)
        self._get_file(Model.ASSUME, assume, year).add_column(self._preparer, group, year, column)
        self._get_file(Model.HISTORICAL, history, year).add_column(self._preparer, group, year, column)
